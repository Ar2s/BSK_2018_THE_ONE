...
/**
 * An implementation of Spray and Focus DTN routing as described in <em>Spray
 * and Focus: Efficient Mobility-Assisted Routing for Heterogeneous and
 * Correlated Mobility</em> by Thrasyvoulos Spyropoulos et al.
 * 
 */
public class SprayAndFocusRouter extends ActiveRouter {
	...
	public void changedConnection(Connection con) {
		 ...
		 * original Epidemic paper), which are exchanged between hosts when a connection
		 * is established. This functionality is already handled by the simulator in the
		 * protocol implemented in startTransfer() and receiveMessage().
		 * 
		 ...
		 * connection access to the peers we recently encountered; so we duplicate the
		 * recentEncounters Map and attach it to a message.
		 */
	...
	public Message messageTransferred(String id, DTNHost from) {
		...
		/*
		 * Here we update our last encounter times based on the information sent from
		 * our peer.
		 */
	...
	protected void transferDone(Connection con) {
		...
		/*
		 * reduce the amount of copies left. If the number of copies was at 1 and we
		 * apparently just transferred the msg (focus phase), then we should delete it.
		 */
	...
	public void update() {
		...
		for (Message m : getMessageCollection()) {
			...
			if (nrofCopies > 1) {
				...
				/*
				 * Here we implement the single copy utility-based forwarding scheme. The
				 * utility function is the last encounter time of the msg's destination node. If
				 * our peer has a newer time (beyond the threshold), we forward the msg on to
				 * it.
				 */
				...
				// for(Connection c : getConnections())
				{
					...
					/*
					 * We need to pick only one peer to send the copy on to; so lets find the one
					 * with the newest encounter time.
					 */
	 ...
	 * host. At the moment, all that's needed is the timestamp of the last time
	 * these two hosts met.
	 */
